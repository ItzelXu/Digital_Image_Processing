...
//（int**）Dithering_matrix(int I_new[], int size)
//{
//	int    i, sum = 0;       
//	double avg;          
//
...
//	}
	...
	for(i = 0; i < Row; i++){
		for(j = 0; j < Column; j++){
			if (0 <= (int)Imagedata[i][j][0] && (int)Imagedata[i][j][0] <= T[i%N][j%N]){
				Imagedata_output[i][j][0] = 0;
			}
			else if (T[i%N][j%N] < (int)Imagedata[i][j][0] && (int)Imagedata[i][j][0] < 256){
